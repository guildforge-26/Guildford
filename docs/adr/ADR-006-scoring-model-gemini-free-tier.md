...
**Model:** `gemini-2.5-flash`, called through a Google AI Studio API key
...
pattern as the old `CLAUDE_MODEL` setting.
...
**Cost tracking kept, now always $0.** `PRICING["gemini-2.5-flash"]` has
...
lowered from 40 to 20 by default. The free tier enforces per-minute AND
per-day request caps (Google's own published numbers for
`gemini-2.5-flash` have varied across sources and over time -- check
ai.google.dev/gemini-api/docs/rate-limits for the current figures rather
than trusting any number quoted here), and exceeding either returns a 429.
`scripts/estimate_cost.py` was rewritten from a dollar-cost estimator into
a call-volume-vs-free-tier-quota estimator to match.
...
- Scoring quality depends on how gemini-2.5-flash performs against
