...
the AI-ops-at-energy-co posting correctly match track4 and pass through?
"""
from jobalerts.prefilter import (
    disqualifying_requirement,
    is_allowed_geography,
    matched_track,
    parse_salary_max_usd,
    run_prefilter,
...
def test_salary_below_minimum_fails():
    assert salary_meets_minimum("$90,000", 100_000) is False


def test_salary_at_or_above_minimum_passes():
    assert salary_meets_minimum("$120,000", 100_000) is True
    assert salary_meets_minimum("$100,000", 100_000) is True


def test_salary_range_passes_if_upper_bound_clears_minimum():
    assert salary_meets_minimum("$90,000 - $130,000", 100_000) is True


def test_salary_hourly_rate_is_annualized():
    # $60/hr * 2080 ~= $124,800 -> clears $100k
    assert salary_meets_minimum("$60/hr", 100_000) is True
    # $20/hr * 2080 = $41,600 -> below $100k
    assert salary_meets_minimum("$20/hr", 100_000) is False


def test_missing_or_unparseable_salary_does_not_block():
    assert salary_meets_minimum(None, 100_000) is True
    assert salary_meets_minimum("competitive", 100_000) is True


def test_parse_salary_max_usd_handles_k_suffix():
    assert parse_salary_max_usd("95K - 120K") == 120_000
...
def test_ai_operations_title_matches_track4(tracks_config):
    assert matched_track("Director of AI Operations", tracks_config) == "track4"


def test_business_development_title_matches_track1(tracks_config):
    assert matched_track("Director of Business Development", tracks_config) == "track1"
...
def test_run_prefilter_drops_posting_under_100k(tracks_config):
    ...
    result = run_prefilter(posting, tracks_config, min_salary_usd=100_000)
...
def test_run_prefilter_drops_posting_requiring_cpa(tracks_config):
    posting = {
        ...
        "salary_text": "$150,000", "raw_text": "Candidates must be a CPA in good standing.",
    }
    result = run_prefilter(posting, tracks_config, min_salary_usd=100_000)
    assert result.passed is False
